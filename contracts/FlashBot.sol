...
contract FlashBot is Ownable {
    ...
    constructor(address _WETHBase) {
        WETH = _WETHBase;
        baseTokens.add(_WETHBase);
        
    }
          /////////////////////////////////just added remove it
    function arbitrargeUSDC(address _tokenAddress, uint256 _amount) public  returns(bool) {
        uint256 arbitraged_amount = (_amount / 10);   
        console.log("arbitraged_amount==>",arbitraged_amount);  
        ...
        )
    {
     ...
     ////////////////////////aave arbitrage//////////////////////////////
     function flashArbitrageAave(address pool0, address pool1 ,uint borrowAmount) external returns (bool) {
    ...
    function pancakeCall(
        ...
        // console.log("before swap pancake",IERC20(info.debtToken).balanceOf(address(this)));
        IUniswapV2Pair(info.targetPool).swap(amount0Out, amount1Out, address(this), new bytes(0));
        // console.log("after swap pancake",IERC20(info.debtToken).balanceOf(address(this)));
        IERC20(info.debtToken).safeTransfer(info.debtPool, info.debtAmount);
        // console.log("after payoffpancake",IERC20(info.debtToken).balanceOf(address(this)));
    }
